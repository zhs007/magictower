...
**内部实现说明**: `MapRender` 内部已经过重构，现在会将所有被标记为 `isEntity: true` 的地图瓦片（例如墙壁）同样创建为 `Entity` 对象进行管理，取代了旧有的、手动的 `wallSprites` 数组。这使得其内部对所有可排序对象的处理逻辑得到了统一。
...
项目新增了一个位于 `apps/mapeditor` 的可视化地图编辑器，用于创建和修改 `mapdata/` 中的地图文件。

### 14.1 功能与架构
- **技术栈**: 前端使用 `Vite` + `TypeScript` + `Pixi.js`，后端使用 `Fastify`。
- **前后端分离**: 编辑器包含一个轻量级的 Fastify 后端，专门负责地图和资产文件的读写，以及一个基于 Pixi.js 的前端界面。
- **实时预览**: 编辑器的核心特性是实时预览。左侧面板的所有修改（无论是编辑瓦片定义还是在网格上绘制）都会立刻在右侧的渲染面板中反映出来。

### 14.2 界面与使用方法
编辑器界面分为左右两部分：

- **右侧 - 渲染面板**:
  - 使用 `@proj-tower/maprender` 包来渲染地图，确保编辑器中的预览效果与游戏中的实际效果完全一致。

- **左侧 - 编辑面板**:
  - **地图选择器**: 顶部的下拉菜单会列出 `mapdata/` 目录下的所有地图，方便加载和切换。
  - **保存按钮**: 将当前的所有修改保存到对应的地图 JSON 文件中。
  - **瓦片资源面板 (Tile Palette)**:
    - **列表**: 显示当前地图定义的所有瓦片（`tileAssets`）。每个瓦片会显示其图片、ID、以及是否为“实体”(`isEntity`)。
    - **添加新瓦片**: 你可以定义一个新的瓦片 ID，从 `assets/map/` 目录下的可用图片列表中选择一张图片，并设置它是否为实体。实体（如墙）会参与 Y 轴排序，实现遮挡效果。
  - **地图网格 (Map Grid)**:
    - 一个动态尺寸的网格，其大小会根据加载的地图数据 (`layout` 数组的尺寸) 自动调整。
    - **绘制**: 从上方的瓦片资源面板中选择一个瓦片，然后在网格上点击即可“绘制”该瓦片的 ID。
  - **新建地图**:
    - 点击“New Map”按钮，可以创建一个新的地图文件。
    - 新地图的默认尺寸为 16x16，该值可在 `apps/mapeditor/.env` 文件中配置。

### 14.3 如何运行
1.  确保已安装所有依赖 (`pnpm install`)。
2.  在项目根目录运行以下命令来启动地图编辑器的开发服务器：
    ```bash
    pnpm --filter @proj-tower/mapeditor dev
    ```
3.  在浏览器中打开提示的 URL (通常是 `http://localhost:5173`)。

## 15. 等级数据编辑器 (Leveldata Editor)

为了方便游戏设计者调整和优化玩家的成长曲线，项目新增了一个位于 `apps/leveldataeditor` 的可视化等级数据编辑器。

### 15.1 功能与架构
- **技术栈**: 前端使用 `Vite` + `TypeScript` + `ECharts`，后端使用 `Fastify`。
- **目的**: 提供一个用于编辑 `gamedata/leveldata.json` 的图形化界面。
- **前后端分离**: 编辑器包含一个轻量级的 Fastify 后端，负责读写 `leveldata.json` 文件。
...
- **左侧 - 表格编辑区**:
  - 以表格形式显示所有等级数据。
  - 用户可以直接在表格中修改除 `level` 之外的所有属性。
  - **保存按钮**: 将所有修改保存回 `gamedata/leveldata.json` 文件。
  - **扩展/收缩按钮**:
    - **Expand Levels**: 允许用户将等级扩展到更高的级别（例如 100 级）。新数据将基于现有等级的平均增长率以线性方式生成。
    - **Shrink Levels**: 允许用户收缩等级数据，删除多余的等级信息。

- **右侧 - 图表区**:
  - 使用 ECharts 渲染一个多系列线图。
  - 直观地展示关键属性（`exp_needed`, `maxhp`, `attack`, `defense`, `speed`）随等级变化的趋势。
  - 图表会在数据保存或修改（扩展/收缩）后自动刷新。

### 15.3 如何运行
1.  确保已安装所有依赖 (`pnpm install`)。
2.  在项目根目录运行以下命令来启动等级数据编辑器的开发服务器：
    ```bash
    pnpm dev:leveldataeditor
    ```
3.  在浏览器中打开提示的 URL (通常是 `http://localhost:5174`)。
...
## 17. 楼层切换系统 (Floor Transition System)
...
## 19. 怪物编辑器 (Monster Editor)

项目新增了一个位于 `apps/monstereditor` 的怪物编辑器，用于编辑怪物数据和模拟战斗。

### 19.1 功能与架构
- **技术栈**: 前端使用 `Vite` + `TypeScript` + `Pixi.js`，后端使用 `Fastify`。
- **前后端分离**: 编辑器包含一个轻量级的 Fastify 后端，负责读写 `gamedata/monsters` 和 `gamedata/leveldata.json` 文件。
- **目的**: 提供一个用于编辑怪物数据和模拟战斗的图形化界面。
...
- **右侧 - 编辑展示区**:
  - **上部 - 选择区**:
    - **玩家配置**: 弹出框中通过下拉框选择玩家等级，并可配置装备（暂未实现）。
    - **怪物选择**: 通过下拉框选择怪物等级和该等级下的怪物。
  - **下部 - 战斗模拟渲染区**:
    - 使用 `@proj-tower/maprender` 包来渲染一个16x16的地图。
    - 玩家和当前选中的怪物并排站在中间。
    - 点击渲染区，即可触发战斗，并通过弹出框显示结果。

### 19.3 如何运行
1.  确保已安装所有依赖 (`pnpm install`)。
2.  在项目根目录运行以下命令来启动怪物编辑器的开发服务器：
    ```bash
