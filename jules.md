...
项目新增了第二版地图生成器，它采用基于模板匹配的算法，能生成风格更多样、结构更可控的地图。

#### a. 核心算法
1.  **模板库 (Template Library)**: 生成器使用一个预设的“房间模板”库。每个模板是一个小型的二维数组，定义了一个房间或房间组合的形状，其中 `0` 是地板, `1` 是墙, `-1` 是留空, `-2` 是潜在的门位置。
2.  **迭代放置 (Iterative Placement)**: 算法在一个循环中执行多次。在每次迭代中，它会遍历所有模板和地图上的所有可能位置，寻找“有效放置点”。
3.  **加权选择 (Weighted Selection)**: 一个放置点是否有效，取决于模板的墙壁是否能与地图上已存在的墙壁重合。重合的墙壁越多，该放置点的“权重”就越高。算法会根据权重进行随机选择，倾向于放置那些能与现有结构融合得更好的模板。
4.  **最终处理 (Finalization)**: 在所有模板放置完毕后，地图上所有 `-2`（潜在的门）会根据设定的概率变成真正的门（即地板）或墙壁。所有未被填充的区域将变成公共区域（地板）。

#### b. 如何使用
1.  **修改参数**: 打开 `scripts/run-gen-map-v2.ts` 文件，修改顶部的 `params` 对象。
    - `Width`, `Height`: 地图的宽高。
    - `forceFloorPos`: 一个坐标数组 `[x, y][]`，强制这些位置必须是地板。
    - `seed`: 随机种子，用于复现生成结果。
    - `doorDensity`: 门密度，一个0到1之间的小数，决定了“潜在的门”有多大几率变成真正的门。
    - `maxPlacementAttempts`: 最大放置尝试次数，控制了地图的填充程度。
...
3.  **查看结果**: 脚本执行成功后，会在 `mapdata/` 目录下生成指定的JSON文件，例如 `generated_map_v2.json`。
