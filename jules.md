...
- **配置数据 (`gamedata/`)**: 使用 JSON 格式。每个对象（怪物、道具）都有一个唯一的 `id`。
- **地图数据 (`mapdata/`)**: 使用 JSON 格式。每层地图是一个二维数组，每个单元格包含 `ground` 和 `entity` 两个图层ID。
...
- **已完成/进行中**:
    ...
    - **`plan003` (部分完成)**: `DataManager` 已创建，可从 JSON 文件加载数据。但与 `GameStateManager` 的集成尚未完成。
    - **`plan010` (进行中)**: 核心的 **装备与增益效果系统** 正在开发中。目前已完成数据结构定义、数据加载以及部分核心战斗逻辑的实现。
...
整体开发将按照上述计划顺序，在完成 `plan010` 的核心逻辑后，继续推进渲染、输入和UI等模块的开发。
