...
- **基本宽度与缩放**: 所有实体（角色、怪物、道具、墙壁）的渲染宽度固定为 `65` 像素（即一个地块的宽度）。图片的高度将根据其原始宽高比 **等比缩放**。
- **美术规范**: 这条规则意味着美术可以通过改变源PNG文件的高度来直接控制角色在游戏中的视觉高度。例如，一个 `65x65` 的图片会显示为一个矮角色，而一个 `65x130` 的图片会显示为一个标准2头身角色。图片的原始宽高比将始终被保留。
- **渲染基准**: 所有图片资源在渲染时将采用 **底部居中** 的对齐方式 (`anchor: 0.5, 1`)。这确保了不同高度的角色脚部都能对齐在正确的地块位置。
