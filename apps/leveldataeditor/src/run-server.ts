...
const start = async () => {
    try {
        ...
        const port = Number(process.env.PORT || 5174); // Using a different port
        ...
        const handler = (req: any, res: any) => {
            const url = req.url || '';
            const handleApi = url.startsWith('/api');
            if (handleApi) {
        ...
        console.log(`Leveldata editor dev server listening on http://${host}:${port}`);
